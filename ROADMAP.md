...
- [x] **Flujograma automático** (pedido por John, 2026-09-02): se dibuja con los títulos de los pasos ya escritos, en su orden, sin reescribir nada. SVG generado en el servidor, sin librería de diagramas, para que salga igual en la ficha y en el PDF impreso. Salta a varias columnas cuando hay muchos pasos. Geometría en `lib/procesos/flujograma.ts`, aparte del render, con pruebas de partido de texto, saltos de columna y límites del lienzo
- [x] **Alcance editable** (pedido por John, 2026-09-02): antes el PDF lo generaba con una frase fija a partir de la gestión, así que no coincidía con el documento original de calidad. Ahora es un campo; si se deja vacío se conserva la frase generada
- [x] **Cada quien elimina sus propios borradores** (pedido por John): creó un borrador y no pudo borrarlo. Un documento publicado sigue necesitando el permiso explícito, ni siquiera su autor lo borra
- [x] **Estructura del Procedimiento alineada con la norma**: Requisitos · Recursos · Definiciones · Indicadores · Condiciones generales · Responsables. Se revierte el ocultamiento del bloque de secciones en procedimientos: John necesita justamente esas secciones
- [x] **Importar los pasos desde el formato en Excel** (Claude-Simon, pedido por John 2026-08-26): botón «Importar desde Excel» dentro del editor, en la sección de pasos. Cada fila con actividad se convierte en un paso con el texto tal como está redactado. Detecta la fila de títulos saltándose el encabezado de logo y control documental, reconoce sinónimos de columna (Actividad/Actividades, Salida/Entregables, Frecuencia/Periodicidad…) sin importar el orden, y reporta las columnas que no usó. Los pasos entran al formulario, no a la base: el líder los revisa y asigna cargos antes de guardar, así el guardado sigue registrando la versión en el historial. Lógica en `lib/procesos/importar-pasos.ts`, aparte de la UI para poder probarla
