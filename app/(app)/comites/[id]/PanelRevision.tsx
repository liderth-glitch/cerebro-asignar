    ...
    <div className="vstack" style={{ gap: 8 }}>
      {compromisos.map(c => (
        <div key={c.id} className="hstack" style={{
          ...
          background: c.estado === 'pendiente' ? 'var(--warning-soft)' : 'var(--surface-sunken)',
          border: c.estado === 'pendiente' ? '1px solid var(--warning)' : '1px solid var(--border)',
        }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {c.notas_revision && (
              <div style={{ fontSize: 11.5, color: 'var(--text-2)', marginTop: 4, fontStyle: 'italic' }}>
                {c.notas_revision}
              </div>
            )}
          </div>
